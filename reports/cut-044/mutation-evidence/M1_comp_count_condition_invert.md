# cut-044 Mutation Anchor M1 — comp_count_condition_invert
...
Flipping met_count inverts the count condition. The sufficient case (COMP-CTL-001 + alice: 3 evidence, evidence_min=3) was previously passing on count; after mutation count reports passed=False, so coverage also fails (since neither is passed) and the case must flip to gap_list.
