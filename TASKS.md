...
## 附录 T — 演示台 v1（咨询单域）+ 检索重测 + 移除对照文档【暂缓】（OEI-013，2026-09-26）

> 任务书：`onyx-lab/OEI-013/TASK.md`（v1，A0–A12）
> 证据：`onyx-lab/OEI-013/evidence/` ｜ 工作区：`onyx-lab/OEI-013/workspace/`
>
> **本附录有一处与任务书不一致，写在最前面**：TASK §3.1 记「用户已拍板：移除该对照文档」。
> cc 在执行这次**不可逆删除**前当面征询，用户改选 **「先不删，只做只读取证」**。
> 因此 **A1 / A2 未按原样完成**（详见 §T.7）。其余验收项照做。

### T.1 一句话结论

把客户面从「能看能测的工装」改成了**一个能讲 10 分钟的咨询演示**（问题 → 知识库 → 你的文件），
并把检索这件事**在产品可见层重新量了一遍** —— 量出来的结论和任务书的预期相反：

- **原引擎层命中率仍是 0%**（两档皆然），与 OEI-012 一致，本刀**没有**修好它；
- **产品可见层（过权限过滤后）原本也是 0%**，因为引擎大多数时候只吐回那份受控对照文档，
  而它会被 fail-closed 挡掉，用户看到的是**空召回组**（36 次原始调用里 21 次为空）；
- **加了一层确定性关键词改写之后，产品层 hit@3 从 0% 升到 44.4%**（9 条 hit 查询口径）；
- **任务书 §3.2.2 建议的「优先用 `skip_query_expansion=true` 确定性路径」实测是有害的**：
  它更可复现（1.000 vs 0.917）也更快，但候选集更窄，在本语料上唯一命中就是那份对照文档，
  过滤后**召回组全空**，产品层 hit@3 掉回 **0%**。因此演示**不采用**该路径（详见 §T.4）。

### T.2 对照文档：决定不删（用户 2026-09-26）

| 项 | 值 |
|---|---|
| 对象 | `ece-df16d19c9e7b-oei009-comparison-restricted.md`（`员工报销政策与发票审核流程`） |
| `user_file_id` | `6138305b-0f40-468f-a1c8-bdea6f68d424` |
| 现状 | **仍在 Onyx 项目 1**（项目 1 = 4 份文件：3 份真文档 + 它） |
| 用户原话 | **先不删，只做只读取证** |
| 本刀做了什么 | 只读取证（`evidence/00`、`evidence/01`）+ 让**演示起链不复建它**（`demo-up.sh` 不调 `step26`） |

**一个意外但重要的副产品**：**不删它，演示也可以是干净的。** OEI-009 的逐条权限过滤是
**fail-closed** 的 —— 引擎文档若在 `engine_documents` 里没有登记行就直接隐藏（`no_registry`）。
本刀的 demo 起链只回填那 3 份真文档，所以对照文档在**产品层从未出现过一次**
（`03-retrieval-filtered.json`：`restricted_doc_visible_runs = 0`，原始 36 次 + 改写路径全算上）。

> 也就是说：**它是"引擎层污染源"，但已经不是"产品层污染源"**。
> 删它仍然值得做（引擎层排序仍然被它挤占），但**不再阻塞演示**。
> 这让"先不删"这个决定**有据可依**，而不是单纯把问题往后推。

### T.3 任务书一处前提落空（如实记录）

TASK §3.2.1 要求评测集「**去掉针对对照文档的 miss 行**」。**这个前提是空的**：

- `data/eval/retrieval/consulting-demo.json` 的 12 条里，`expected_title` **没有任何一条**
  指向对照文档（9 hit + 3 miss，目标全是三份真文档）；
- 唯一相关的是 **q10**，它的 **query 文本**（`员工报销政策与发票审核流程`）与对照文档同题，
  但它是一条 **miss 行**，`expected_title = play-sales-delivery.md`；
- 因此**一行都没有删**，评测集与 OEI-012 **逐字节相同**
  （sha256 `9e5ef6f926383543a6ba4d9b39b8b3817945e8d32353247383505ad0ddf5bf6c`），
  「口径与 012 一致」由此可**机械核对**，而不是靠叙述。
  q10 保留并在 §T.4 的表里单独标出。

### T.4 检索重测（本刀的数字都在这里）

**口径**：`OEI-012/workspace/retrieval_runner.py` + 同一评测集，N=3 × 两档；
产品层用 `workspace/filtered_runner.py`（匿名调用 `/api/v1/consulting/library`，记过滤后的 `engine_items` title 集）。
改写词不是重写的 —— 由 `workspace/rewrite_probe.js` **从发版 `app.js` 里读出来**。

**(a) 原始引擎层**（`evidence/02-retrieval-rematrix.json`，72 次调用，wall 307.9s）
...
> 与 OEI-012 的差别本身值得记：012 的不稳定查询是 **{q09, q11}**，本刀是 **{q11}** ——
> **"哪些查询不可复现"这件事自己就不稳定**。两刀唯一一致的是 **q11**。
> 所以"复现性 0.833 vs 1.000"是**两次测量各自的事实**，不宜当成常数。

**(b) 产品可见层**（`evidence/03-retrieval-filtered.json` 默认档 / `03b-…-deterministic.json` 确定性档）

| 路径 | 档位 | hit@1 | hit@3 | 召回组为空 | 对照文档可见 | 可复现 |
|---|---|---|---|---|---|---|
| 原始问句 | 默认（expansion on） | 0.0% | 0.0% | 21/36 | **0** | 0.917 |
| 原始问句 | 确定性（expansion off） | 0.0% | 0.0% | 21/36 | **0** | 1.000 |
| 关键词改写 | 默认 | **40.0%** | **80.0%** | 1/5 | **0** | — |
| 关键词改写 | 确定性 | 0.0% | 0.0% | **5/5** | **0** | — |

**演示实际走的路径**（有改写就用改写，没有就用原始问句）逐条：
...
⇒ **9 条 hit 查询上，演示路径 hit@1 = 2/9 = 22.2%，hit@3 = 4/9 = 44.4%**（原始路径 0% / 0%）。

**四条诚实的边界**（不许被读成"修好了"）：

1. 44.4% 仍然**不是**可演示的召回质量，只是从 0% 抬起来了。
2. 抬起来的原因**有一部分不是检索变好**，而是**权限过滤把对照文档挡掉**，
   真文档因此从第 2、3 位浮上来 —— 这是 OEI-012 VERDICT §5 预告过的效应，本刀量到了。
3. 5 条查询**不会触发改写**（问句里没有词表里的业务词），它们仍是 0%。
4. 产品层的"词表 + 固定规则改写"是**兜底**，不是检索能力本身。
   API 层 `/api/v1/consulting/library` 对**所有其它调用方**仍做整串匹配。

### T.5 与任务书 §3.2.2 相反的一条结论（本刀最重要的负面发现）

TASK §3.2.2 写：「若仍差 → 本刀检索部分只做到把确定性、可复现的路径接进演示：
**优先用 `skip_query_expansion=true` 的确定性向量路径**」。

**实测不支持这个优先级。** 确定性档确实更可复现（1.000 vs 0.917）也更快（1.518s vs 4.567s median），
但在产品层它是**净负**的：改写路径的召回组**5/5 全空**，hit@3 **0% vs 默认档 80%**。
机制很清楚：确定性档返回的候选集更窄，这些问句下它唯一返回的就是那份对照文档，
而对照文档被 fail-closed 挡掉 → 用户看到空。

**所以本刀的选择是：演示链默认走引擎默认档（`ECE_LIBRARY_SKIP_QUERY_EXPANSION=0`）。**
开关本身按 §3.2.2 接了进来（服务端环境变量，见 `docs/API.md` §10），
**但默认值是按数字选的，不是按任务书的建议选的**，理由写进 `demo-up.sh` 的注释。

> 这正是 OEI-012 VERDICT §6 那条"负面发现也是交付"的延续：
> 任务书给了一条路径，实测说这条路径在**产品层**是亏的 —— 那就报出来，并按数字选。

### T.6 演示面交付物

| 交付物 | 位置 | 说明 |
|---|---|---|
| 视图 D 三幕 | `ece/demos/spa/{index.html,app.js,styles.css}` | ① 一个问题 ② 知识库怎么答 ③ 你的文件；**既有 DOM 锚点与测试断言一个没改**（`evidence/04`） |
| 确定性关键词改写 | `ece/demos/spa/app.js` | 49 词表，纯字符串规则，无模型；**改写结果对观众可见**；同问句 5/5 次结果一致 |
| 演示脚本 | `onyx-lab/OEI-013/workspace/demo-script.md` | 10–15 分钟、三幕、客户语言、含「我们做了什么 / 边界 / 下一步」 |
| 常驻起链 | `onyx-lab/OEI-013/workspace/demo-up.sh` | 一条命令；幂等（连跑两次第二次不重复起，`evidence/06`）；内置 `no_proxy`；打印浏览器 URL |
| 停 / 清 | `workspace/demo-down.sh` | 默认只停 api + origin；`--all` 再删演示 PG |
| `make` 入口 | `ece/Makefile` | `make demo-up` / `make demo-down` |

**停 / 清一句话**：`bash onyx-lab/OEI-013/workspace/demo-down.sh`（连带清库加 `--all`）。

### T.7 验收偏差（需要 codex 关注）

| # | 验收 | 状态 | 说明 |
|---|---|---|---|
| **A1** | 两步删除 + 删后项目 1 只剩 3 份 | ❌ **未按原样完成** | 用户改选「先不删」。**没有发出任何 DELETE**；项目 1 仍 4 份。只读证据在 `evidence/00`、`evidence/01`，含未来若要删的**确切命令与标识符** |
| **A2** | 起链不复建 + 起链后仍 3 份 | ⚠️ **一半成立** | "不复建"**成立且已证**：`demo-up.sh` 不调 `step26`，起链后项目 1 仍是 4 份（没有第 5 份）。"仍 3 份"不成立，因为 A1 没做 |
| A3 | N=3 × 两档（原始 + 过滤后）hit/复现/延迟如实落盘 | ✅ | `02`（原始，两档）+ `03`/`03b`（过滤后，两档）；口径与 012 逐字节一致 |
| A4 | 结论诚实；若不改善则接确定性路径 | ✅ **但结论与任务书相反** | 见 §T.5：确定性路径实测有害，故**不接进演示**，理由与数字写进 `demo-up.sh` 与 REPORT |
| A5 | 视图 D 三段可见 + 机器可校验 + 未迁就 UI 改断言 | ✅ | `evidence/04`（HTML 转储 + 锚点 + API 原始输出；无截图）；`tests/` 既有断言零改动 |
| A6 | 演示脚本 10–15 分钟三幕 | ✅ | `workspace/demo-script.md` |
| A7 | 起链一条命令 / 幂等 / no_proxy / 打印 URL；**留现场** | ✅ | `evidence/06`；链子已起并留驻 |
| A8 | 契约不回归；`/library`、`/facets` 键集合不变；全套件 0 failed | ✅ | `evidence/07`、`evidence/08` |
| A9 | `docs/API.md`、`TASKS.md 附录 T`、`Makefile`；`check-api-docs` App-only 0 | ✅ | `evidence/11` |
| A10 | commit（不 push）+ 短 hash + 文件清单 | ✅ | `evidence/09` |
| A11 | 合规与资源 | ✅ | `evidence/10`（**demo 链不拆**，评测临时 PG 已释放） |
| A12 | 证据纪律 | ✅ | 全部机器可校验；无截图 |

### T.8 转出

1. **对照文档去留**：产品层已不受它影响，但**引擎层排序仍被它挤占**。等用户看过数字后决定
   （`evidence/00` 里备好了两步删除的确切命令）。
   注意 `onyx-lab/OEI-009/workspace/step26_demo_falsifiable.py` **仍在**，谁跑它谁就会把它重建 ——
   它是 OEI-009 的历史证据工具，本刀**无权改**（§8 未授权 `onyx-lab/OEI-009/`），这是**已知残留风险**。
2. **排序问题是最大遗留**（承 OEI-012）：hit@k 在产品层靠"过滤 + 改写"抬到 44.4%，
   但**引擎本身仍是 0%**。要真修得动 Onyx 服务端配置或换检索路径 —— 超出本刀授权。
3. **目录检索语义化**：现在靠 49 词表 + 固定规则兜底；把整串匹配换成真正的检索是 API 契约级改动。
4. **真认证**（承 OEI-009）：演示里讲"两种身份、不同召回"之前必须先做。
